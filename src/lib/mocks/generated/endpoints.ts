...
/**
 * @summary Logout everywhere (revoke all refresh tokens for current user)
 */
...
export type logoutAllResponseError = (logoutAllResponse401) & {
  headers: Headers;
};
...
/**
 * @summary Get tasks by user ID (paged; default page=0, size=20)
 */
...
export const getGetTasksByUserIdUrl = (userId: number,
  ...
  return stringifiedParams.length > 0 ? `/api/tasks/user/${userId}?${stringifiedParams}` : `/api/tasks/user/${userId}`
}

export const getTasksByUserId = async (userId: number,
    params?: GetTasksByUserIdParams, options?: RequestInit): Promise<getTasksByUserIdResponse> => {
  
  const res = await fetch(getGetTasksByUserIdUrl(userId,params),
  {      
    ...options,
    method: 'GET'
...
/**
 * @summary Get tasks by user ID and date range (paged; default page=0, size=20)
 */
